...
// ✅ Importamos a nova função 'updateCartItem'
import { getCartItems, removeCartItem, updateCartItem } from '../api';
import './Carrinho.css';

// Componente para o ícone de lixeira (SVG)
const LixeiraIcon = () => (
...
function Carrinho() {
  ...
  // ✅ Função para REMOVER um item (passando o ID do item do carrinho, não do produto)
  ...
  if (loading) {
    return <div className="carrinho-container"><h2>A carregar pedido...</h2></div>;
  }
      ...
      <div className="lista-produtos">
        ...
        {itens.map(item => (
          <div key={item.id} className="carrinho-item">
            <img src={item.produto.imagem || '/placeholder-food.jpg'} alt={item.produto.nome} className="item-imagem" />
            <div className="item-detalhes">
              <h3>{item.produto.nome}</h3>
              <p className="item-preco">R$ {parseFloat(item.produto.preco).toFixed(2)}</p>
            </div>
            <div className="item-acoes">
              ...
              <strong className="item-subtotal">R$ {(parseFloat(item.produto.preco) * item.quantidade).toFixed(2)}</strong>
